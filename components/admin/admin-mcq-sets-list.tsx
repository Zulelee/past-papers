...
export default function AdminMcqSetsList({ mcqSets = [] }) {
  const [searchTerm, setSearchTerm] = useState("")
  const [boardFilter, setBoardFilter] = useState("all")
  const [classFilter, setClassFilter] = useState("all")
  const router = useRouter()

  // Filter mcqSets based on search term, board filter, and class filter
  const filteredMcqSets = mcqSets.filter((set) => {
    ...
    const matchesClass = classFilter === "all" || set.class === classFilter
                ...
                <TableRow key={set.id}>
                  ...
                  <TableCell>
                    ...
                    <div className="md:hidden text-xs text-muted-foreground mt-1">
                      {set.board} • {set.class ? `${set.class}th Class` : "N/A"} • {set.subject} • {set.year}
                    </div>
                  </TableCell>
                  <TableCell className="hidden md:table-cell">{set.board}</TableCell>
                  <TableCell className="hidden md:table-cell">{set.class ? `${set.class}th` : "N/A"}</TableCell>
                  ...
                  <TableCell className="hidden md:table-cell">
                    {typeof set.mcq_count === "object" ? set.mcq_count.count : set.mcq_count || 0}
                  </TableCell>
                  <TableCell>
                    <DropdownMenu>
