...
import { getMcqSetById, getMcqsBySetId, incrementPracticeCount } from "@/app/actions/mcq-actions"

export default function McqPracticePage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const setData = await getMcqSetById(params.id)
        const mcqsData = await getMcqsBySetId(params.id)

        setMcqSet(setData)
        setMcqs(mcqsData)

        // Track practice
        await incrementPracticeCount(params.id)
      } catch (error) {
        console.log("Error loading MCQs:", error)
      } finally {
        setLoading(false)
      }
    }
    ...
    return (
      <div className="container py-8 flex justify-center items-center min-h-[50vh]">
        <div className="text-center">
          ...
          <p className="mt-4 text-muted-foreground">Loading MCQs...</p>
        </div>
      </div>
    )
  }

  if (!mcqSet || mcqs.length === 0) {
    return (
        ...
        <div className="flex flex-col gap-6 max-w-2xl mx-auto">
          ...
          <Card className="text-center py-12">
            <CardContent>
              ...
              <p className="text-muted-foreground mb-6">There are no MCQs available for this practice session yet.</p>
              <Button asChild>
  ...
  if (completed) {
    ...
    return (
      <div className="container py-8 max-w-2xl mx-auto">
        <Card>
          <CardHeader>
            <CardTitle className="text-xl text-center">Practice Complete</CardTitle>
          </CardHeader>
