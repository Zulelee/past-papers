...
export default function NewMcqPage({ params }: { params: { id: string } }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("paper_id", params.id)
      ...
      if (result.success) {
        ...
        router.push(`/admin/papers/${params.id}/mcqs`)
        router.refresh()
      } else {
      ...
      <div className="flex flex-col gap-6 max-w-3xl mx-auto">
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="sm" asChild>
            <Link href={`/admin/papers/${params.id}/mcqs`}>
        ...
        <div className="flex flex-col gap-2">
          ...
          <p className="text-muted-foreground">Create a new multiple-choice question for this paper.</p>
        </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <CardFooter className="flex justify-between">
              <Button variant="outline" asChild>
                <Link href={`/admin/papers/${params.id}/mcqs`}>Cancel</Link>
              </Button>
